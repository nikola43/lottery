...
declare_id!("Gwz4oGEDjPwcFQz5FzihtCD8joME1cRAXBpCaowMEHhL");
...
pub mod lottery {
    ...
    pub fn create_lottery(
        ...
        if price == 0 || ticket_amount <= 0 || prize_amount == 0 {
        ...
        lottery.left_prize = 0;
        lottery.prize_bump = prize_bump;
        lottery.proceeds_bump = proceeds_bump;
        Ok(())
    }

    pub fn add_prize(ctx: Context<AddPrize>) -> Result<()> {
        let lottery = &mut ctx.accounts.lottery;
        let amount = lottery.prize_amount;
        lottery.left_prize = amount;
    ...
    pub fn claim_prize(ctx: Context<ClaimPrize>) -> Result<()> {
        ...
        let binding: Pubkey = ctx.accounts.lottery.key();
